...
namespace btree      // бинарное дерево, не допускается дублирование ключей
{
	...
	bool countRoot(Node* current, int key, int count)
	{
		...
		if (current != nullptr)
		{
			...
			{
				std::cout << "количество ветвей: " << count << std::endl;
				return true;
			}
			if (key < current->data->key)
				return countRoot(current->left, key, count + 1);
			else
				return countRoot(current->right, key, count + 1);
		}
		return false;
	}
	void mixedTraversal(Node* current, int level) {
		if (current == nullptr) {
			return;
		}

		mixedTraversal(current->left, level + 1);
		for (int i = 0; i < level; i++)
			std::cout << " ";
		std::cout << current->data->key << std::endl;
		mixedTraversal(current->right, level + 1);
	}
	void preOrderTraversal(Node* current, int level) {
		if (current == nullptr) {
			return;
		}
		for (int i = 0; i < level; i++)
			std::cout << " ";
		std::cout << current->data->key << std::endl;
		preOrderTraversal(current->left, level + 1);
		preOrderTraversal(current->right, level + 1);
	}
}
